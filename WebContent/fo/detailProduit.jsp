...
<div id="content" class="inner clearfix">
	<div id="breadcrumb">
		...
		<c:if test="${produit.isTypeAlbum() == false}">
			<c:if test="${produit.getAlbum() != null}">
	...
	<div id="primary" class="clearfix" role="main">
		...
		<div id="product-${produit.getId()}" class="post-${produit.getId()} product type-product status-publish hentry">
			<div class="images">
				<a href="photoProduit.action?id=${produit.getId()}" class="zoom" rel="thumbnails" title="${produit.getNom()}">
					<img width="306" height="306" src="photoProduit.action?id=${produit.getId()}&source=Front" class="attachment-shop_single wp-post-image" alt="${produit.getNom()}" title="${produit.getNom()}" />
				</a>
				<div class="thumbnails"></div>
			</div>
			<div class="summary">
				<h1 class="product_title entry-title">${produit.getNom()}</h1>
				<div>
					<p class="price">
				...
				<div class="cart">
				 	<button type="button" class="add_to_cart_button button product_type_simple" data-product_id="${produit.getId()}" onclick="productAdded('${produit.getId()}')">Ajouter au panier</button>
				</div>
					...
					<ol class="commentlist">
						<li itemprop="reviews" itemscope="" itemtype="http://schema.org/Review" class="comment byuser comment-author-davidvassett bypostauthor even thread-even depth-1" id="li-comment-4">
							<div id="comment-4" class="comment_container">
								<img alt="" src="./Front-Office/assets/placeholder.png" class="avatar avatar-60 photo" height="60" width="60"/>
